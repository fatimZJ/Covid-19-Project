...
simulateOutbreakSEIcIscR = function(R0t,rho=c(rep(0.4,4),rep(0.8,12)), 
                                    R0tpostoutbreak = 1.5,
                                    ...
                                    contacts_ireland=contacts)
{
  ...
  # dateStart = as.Date('2019-12-01')                     # included as a function argument 
  dateEnd = dateStart+(tmax-1)
  dateStartCNY = as.Date('2020-01-25')                   # start date of chinese new year
  dateEndCNY = as.Date('2020-01-31')                     # end date of chinese new year
  ...
  # Initialise the time-dependent variables, i.e. setting the values of the variables at time 0
  E[1,] = 0 
  Ic[1,] =  pInfected*sum(N_age)/16 # Initial number of infected people # Assign 100 infected person in each age group (TODO RELAX?)
  Isc[1,] = 0 
  R[1,] = 0 
  ...
  ## INTERVENTIONS 
  # School closed 2020-02-10, 
  #lockdown (intense intervention) started 2020-01-23, end of intense intervention: user-specified 
  # note that intense intervention is time-varying control by pWorkOpen: proportion of the work force that is working
  # debug pWorkOpen = c(0.1,0.25,0.5,1)
  ...
  { 
    ...
    numEtoIc  = alpha*rho*E[stepIndex,]*dt;                           # E to Ic  (alpha here is kappa in paper)
    ...
    incidence[stepIndex+1,] = numEtoIc/dt;     # Only the clinical cases are included in the incidence per day
