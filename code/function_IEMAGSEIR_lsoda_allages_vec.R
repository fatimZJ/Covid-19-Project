...
length(times)
## interpolating R
R0_int <- approx(jg_dat$Rt, xout = seq(from = 1, to = tmax, by = dt))$y
length(R0_int)
#is.na(R0_int)
...
xstart_df <- tibble(
  names = c(paste0('S_',1:groups),
            ...
            paste0('Iq_',1:groups), 
            paste0('R_',1:groups)), 
  value = c(c(N_age - num_inf),# - num_rec)# - num_exp),
...
# turn it into a named vector (required input by desolve)
dim(xstart_df)
...
#plot(xx,(rowSums(cbind(Ip, IA, Ii,It,Iti,Iq))), lwd = 2, col = "red", type = "l")
...
sol <- ode(xstart, times, SEIR_model, parms ,"bdf")
