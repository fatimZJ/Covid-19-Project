  ...
  tabItems(
    ...
    tabItem(tabName = "seir", 
            fluidRow(
              ...
              tabBox( width = 10, title = "Overall SEIR Output",
                      tabPanel( title = "Dublin",
                        plotlyOutput("summary_plot") %>% 
                          withSpinner(color = "#0dc5c1", size = 2, hide.ui = FALSE)),
                      tabPanel(title = "Ireland", plotlyOutput("summary_plot_ie") %>% 
                                 withSpinner(color = "#0dc5c1", size = 2, hide.ui = FALSE))
              ),
              tabBox( width = 10,
                title = "Age Group and Compartment Breakdown",
                tabPanel(title = "Dublin",
                  fluidRow(
                    column(6,
                ...
                tabPanel(title = "Ireland",
                         fluidRow(
                           column(6,
                                  plotlyOutput("S_age_plot_ie") %>% 
                                    withSpinner(color = "#0dc5c1", size = 1, hide.ui = FALSE)
                           ),
                           column(6, 
                                  plotlyOutput("E_age_plot_ie") %>% 
                                    withSpinner(color = "#0dc5c1", size = 1, hide.ui = FALSE)
                           ),
                           column(6, 
                                  plotlyOutput("I_age_plot_ie") %>% 
                                    withSpinner(color = "#0dc5c1", size = 1, hide.ui = FALSE)
                           ),
                           column(6, 
                                  plotlyOutput("R_age_plot_ie") %>% 
                                    withSpinner(color = "#0dc5c1", size = 1, hide.ui = FALSE)
                           )
                         )
